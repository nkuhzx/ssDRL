...
class CADRL(Policy):
    ...
    def propagate(self, state, action):
        if isinstance(state, ObservableState):
            ...
            next_state = ObservableState(next_px, next_py, action.vx, action.vy, state.radius,state.hr_social_stress,
                                         state.eye_contact,state.intention,state.id)
            ...
            if self.kinematics == 'holonomic':
                ...
                next_state = FullState(next_px, next_py, action.vx, action.vy, state.radius,
                                       state.gx, state.gy, state.v_pref, state.theta,state.hr_social_stress)
            else:
                ...
                next_state = FullState(next_px, next_py, next_vx, next_vy, state.radius, state.gx, state.gy,
                                       state.v_pref, next_theta,state.hr_social_stress)
        else:
    ...
    def rotate(self, state):
        ...
        """
        # 'px', 'py', 'vx', 'vy', 'radius', 'gx', 'gy', 'v_pref', 'theta','robot_stress','px1', 'py1', 'vx1', 'vy1', 'radius1','hr_socail_stress'
        ...
        vx1 = (state[:, 11+1] * torch.cos(rot) + state[:, 12+1] * torch.sin(rot)).reshape((batch, -1))
        vy1 = (state[:, 12+1] * torch.cos(rot) - state[:, 11+1] * torch.sin(rot)).reshape((batch, -1))
        px1 = (state[:, 9+1] - state[:, 0]) * torch.cos(rot) + (state[:, 10+1] - state[:, 1]) * torch.sin(rot)
        px1 = px1.reshape((batch, -1))
        py1 = (state[:, 10+1] - state[:, 1]) * torch.cos(rot) - (state[:, 9+1] - state[:, 0]) * torch.sin(rot)
        py1 = py1.reshape((batch, -1))
        radius1 = state[:, 13+1].reshape((batch, -1))
        radius_sum = radius + radius1
        da = torch.norm(torch.cat([(state[:, 0] - state[:, 9+1]).reshape((batch, -1)), (state[:, 1] - state[:, 10+1]).
                                  reshape((batch, -1))], dim=1), 2, dim=1, keepdim=True)
        #robot_com_stress=state
        robot_social_stress=state[:,9].reshape((batch,-1))
        human_stress_index = state[:, 14+1].reshape((batch, -1))

        new_state = torch.cat([dg, v_pref, radius, vx, vy,robot_social_stress, px1, py1, vx1, vy1, radius1, da, radius_sum, human_stress_index],dim=1)
